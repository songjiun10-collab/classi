        ...
        with fitz.open(path) as doc:
            for page in doc:
                ...
                if len(t) < 30:  # 텍스트 레이어 부실 → OCR
                    ...
                    pix = None  # Pixmap의 C측 버퍼를 다음 페이지 전에 즉시 해제(item 1)
                parts.append(t)
        return "\n".join(parts)
...
def _mermaid(a: Analysis) -> str:
    ...
    lines = ["flowchart TD"]
    ids = []
    for i, n in enumerate(a.flow):
        # 노드 id를 인덱스로 고정한다. 모델이 준 n.id를 정규화해 쓰면 'S1'과 'S-1'이 같은
        # 'n_S1'으로 충돌해 서로 다른 단계가 한 노드로 합쳐지고 엣지가 엉킨다.
        nid = f"n_{i}"
        ids.append(nid)
